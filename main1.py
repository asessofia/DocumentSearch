# eMIP Sr Engineering Manager 
# Technical Assessment Case Studies
# Author: Abel A Sessofia
##############################################################################################################
...
class SearchFile:
    ...
    def search_really_large_files(self, start=0):
        ...
        You should take care that you set bsize value such that your RAM can handle it.
        This will prevent memory crash in case file is too large to be handled by the RAM.
        :param start: Starting pointer of file, in our case always 0.
        :return: None
        """
    ...
    def preprocess(self):
        ...
        It will store all the words from all files and store it in a dictionary structure with the number of occurrences
        When user makes the actual search instead of searching into this file it will actually refer to the already
        derived data and returns search results really fast.
        This preprocessing is needed if you are using the method "search_preprocessed_data"
        :return: None
        """
        file_dict = {}
        for file in self.file_generator():
    ...
    def search_preprocessed_data(self):
        ...
        This is the fastest method because it does not actually perform the search every time but refers to values
        which are already available in a data structure
        :return: None
        """
    ...
    def file_generator(self):
        """
        This is a simple generator used to get path of all files in input_dir. It is used by other methods to read files
        :return: yields path of each and every file in directory so you can loop over it.
        """
...
if __name__ == '__main__':
    ...
    # Uncomment this section below to make random searches and test perfomance
